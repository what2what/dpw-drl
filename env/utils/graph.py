...
def binary_search(list_, target):
    # 是一个辅助函数，用于在有序列表 op_unfinished 中快速查找节点 ID 的位置（用于删除节点）
...
class Graph:
    def __init__(self, args, job_num, machine_num):
        ...
        # op-m 边（操作-机器关联）
        self.op_edge_idx = torch.empty(size=(1,0), dtype=torch.int64)                           # for op<->m op 节点索引
        self.m_edge_idx = torch.empty(size=(1,0), dtype=torch.int64)                            # for op<->m  m 节点索引
        self.edge_x = torch.empty(size=(1, 0), dtype=torch.int64)                               # 边特征：加工时间
        ...
        self.op_num = 0                     # 当前图中操作总数
        self.op_unfinished = []             # 未完成的操作节点ID列表（有序）
        self.current_op = []                # 每个作业当前处理到第几个操作（索引）

        self.max_process_time = 0.          # 用于归一化
    ...
    def get_data(self):
        ...
        # 构建边：注意 edge_index 形状应为 (2, num_edges)
